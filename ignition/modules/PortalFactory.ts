...
export default buildModule("PortalFactoryModule", (m) => {
  ...
  const curvyAggregatorAlphaProxyAddress = getDeployedContractAddressOnNetwork(
    aggregatorNetwork,
    "CurvyAggregatorAlpha#ERC1967Proxy",
  );

  const portalFactory = m.contract(
    "PortalFactory",
    [owner, curvyVaultProxyAddress, curvyAggregatorAlphaProxyAddress, aggregatorChainId],
    { id: "PortalFactory" },
  );
