...
library SignatureVerifier {
    /// @dev Thrown when a signed gateway response is used past its `expires` timestamp.
     ...
     * @param expires: The timestamp at which the response becomes invalid.
     ...
     * @param request: The original request that was sent.
     * @param response: An ABI encoded tuple of `(bytes result, uint64 expires, bytes sig)`, where `result` is the data
     *        to return to the caller, and `sig` is the (r,s,v) encoded message signature.
