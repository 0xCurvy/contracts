...
contract CurvyAggregator_operatorExecuteInvalidProofs_FuzzTest is BoilerPlate {
    function testFuzz_processWrapsInvalidProof(
        uint256[2] memory a,
        uint256[2][2] memory b,
        uint256[2] memory c,
        uint256[152] memory inputs
    ) public {
        // Note: there's a small chance that the proof verification and inputs will be valid
        vm.startBroadcast(operator);
        vm.expectRevert();
        curvyAggregator.processWraps(CurvyAggregator_Types.WrappingZKP({a: a, b: b, c: c, inputs: inputs}));
    }
}
