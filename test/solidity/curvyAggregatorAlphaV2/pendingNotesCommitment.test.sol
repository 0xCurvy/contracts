...
import { CurvyPendingNotesCommitmentVerifier } from "../../../contracts/aggregator-beta/verifiers/CurvyPendingNotesCommitmentVerifier.sol";
import { BATCH_SIZE, TREE_DEPTH, PendingNotesCommitmentFixtures } from "./fixtures/pendingNotesCommitment.fixtures.sol";
import { ICurvyAggregatorAlpha } from "../../../contracts/aggregator-beta/ICurvyAggregatorAlpha.sol";

contract PendingNotesCommitmentTest is Test {
    using stdStorage for StdStorage;

    CurvyAggregatorAlphaV2 public curvyAggregatorAlphaV2;
    CurvyPendingNotesCommitmentVerifier public curvyPendingNotesCommitmentVerifier;
    ...
    function setUp() public {
        ...
        curvyAggregatorAlphaV2.setPendingNotesCommitmentVerifier(BATCH_SIZE, TREE_DEPTH, address(curvyPendingNotesCommitmentVerifier));

        PendingNotesCommitmentFixtures.Input memory fixture = PendingNotesCommitmentFixtures.input();

        assertEq(curvyAggregatorAlphaV2.getPendingNotesCommitmentVerifier(BATCH_SIZE, TREE_DEPTH), address(curvyPendingNotesCommitmentVerifier));
        assertEq(curvyAggregatorAlphaV2.getCurrentNotesTreeRoot(), fixture.currentNotesRoot);
        assertEq(curvyAggregatorAlphaV2.getCurrentNotesBatchIndex(), 0);
        assertEq(curvyAggregatorAlphaV2.getCurrentNoteIndex(), fixture.currentNoteIndex);
    }

    function test_commitPendingNotes() public {
        console.log("> Testing commitment of two pending notes and three dummy notes");
        PendingNotesCommitmentFixtures.Input memory fixture = PendingNotesCommitmentFixtures.input();
        ...
        assertTrue(
            curvyPendingNotesCommitmentVerifier.verifyProof(
                fixture.proof_a, fixture.proof_b, fixture.proof_c, fixture.public_signals
            ),
        ...
        assertEq(curvyAggregatorAlphaV2.getCurrentNotesTreeRoot(), fixture.newNotesRoot);
        assertEq(curvyAggregatorAlphaV2.getCurrentNoteIndex(), fixture.newNoteIndex);
    ...
    function _setNotePending(uint256 noteId) internal {
        stdstore.target(address(curvyAggregatorAlphaV2)).sig("noteStatus(uint256)").with_key(noteId).checked_write(
