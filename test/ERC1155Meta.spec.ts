...
import { BigNumber, type ContractTransaction, type EventFilter, utils } from "ethers";
...
import type { ApprovalSignature, BatchTransferSignature, GasReceipt, TransferSignature } from "src/typings/tx-types";
import {
  AbstractContract,
  createTestWallet,
    ...
    conditions.forEach((condition) => {
      context(condition[2] as string, () => {
        ...
        it("should REVERT if data is random", async () => {
          const dataUint8 = utils.toUtf8Bytes(
        ...
        it("should REVERT if transfer data is incorrect", async () => {
          ...
          if (isGasReceipt) {
            goodGasAndTransferData = utils.defaultAbiCoder.encode(
        ...
        it("should REVERT if nonce is incorrect", async () => {
          ...
          data = await encodeMetaTransferFromData(transferObj, domainHash, gasReceipt);
          await operatorERC1155Contract.metaSafeTransferFrom(
            ownerAddress,
            receiverAddress,
            id,
            amount,
        ...
        describe("ERC-1271 Receiver", () => {
          ...
          beforeEach(async () => {
            ERC1271WalletValidationMockAbstract =
              await AbstractContract.fromArtifactName("ERC1271WalletValidationMock");
        ...
        describe("When signature is valid", () => {
          ...
          it("should REVERT if invalid response from receiver contract", async () => {
            ...
            await expect(tx).to.be.rejectedWith(
              /ERC1155#_callonERC1155Received: INVALID_ON_RECEIVE_MESSAGE|out of gas/,
            );
          });
          ...
          describe("When gas is reimbursed", () => {
            ...
            it("should send gas fee to msg.sender is fee recipient ix 0x0", async () => {
              ...
              await receiverERC1155Contract.metaSafeTransferFrom(
                ownerAddress,
                receiverAddress,
                id,
                amount,
            ...
            it("should REVERT if gasReceipt is incorrect", async () => {
              ...
              // Correct and incorrect transferData
              const goodGasAndTransferData = utils.defaultAbiCoder.encode(
                [GasReceiptType, "bytes"],
                [gasReceipt, transferData],
              );
              const badGasAndTransferData = utils.defaultAbiCoder.encode(
                [GasReceiptType, "bytes"],
          ...
          context("When successful transfer", () => {
            ...
            describe("TransferSingle event", async () => {
              ...
              it("should have `msg.sender` as `_operator` field, not tx.origin", async () => {
                ...
                // @ts-expect-error (https://github.com/ethereum-ts/TypeChain/issues/118)
                ...
                // @ts-expect-error (https://github.com/ethers-io/ethers.js/issues/204#issuecomment-427059031)
    ...
    conditions.forEach((condition) => {
      context(condition[2] as string, () => {
        ...
        it("should REVERT if nonce is incorrect", async () => {
          ...
          transferObj.nonce = nonce;
          data = await encodeMetaBatchTransferFromData(transferObj, domainHash, gasReceipt);
          await operatorERC1155Contract.metaSafeBatchTransferFrom(
            ownerAddress,
            receiverAddress,
            ids,
            amounts,
        ...
        describe("When signature is valid", () => {
          ...
          it("should REVERT if sending to 0x0", async () => {
            ...
            await expect(tx).to.be.rejectedWith(
              RevertError("ERC1155Meta#metaSafeBatchTransferFrom: INVALID_RECIPIENT"),
            );
          });
  ...
  describe("metaSetApprovalForAll() function", () => {
    ...
    conditions.forEach((condition) => {
      context(condition[1] as string, () => {
        ...
        it("should REVERT if contract address is incorrect", async () => {
          domainHash = utils.keccak256(
            utils.solidityPack(["bytes32", "uint256"], [DOMAIN_SEPARATOR_TYPEHASH, receiverAddress]),
          );
          data = await encodeMetaApprovalData(approvalObj, domainHash, gasReceipt);
        ...
        it("should REVERT if nonce is incorrect", async () => {
          ...
          data = await encodeMetaApprovalData(approvalObj, domainHash, gasReceipt);
          await operatorERC1155Contract.metaSetApprovalForAll(
            ownerAddress,
            operatorAddress,
            approved,
            isGasReimbursed,
