...
} from "../../src/aggregator/Exports.sol";
import {MockERC20} from "./mocks/MockERC20.sol";
import {MockActionHandler} from "./mocks/MockActionHandler.sol";

contract Integration_BoilerPlate is Test {
    function basicSetUp() public {
        ...
        m20 = new MockERC20[](N_ERC20_TOKENS);
        for (uint256 i = 0; i < N_ERC20_TOKENS; ++i) {
            m20[i] = new MockERC20((i + 1) * 10 ** 20);
        }
        vm.stopBroadcast();
        ...
        CSUC_Types.ConfigUpdate memory _csucConfig = _buildConfig();

        address proxy = Upgrades.deployUUPSProxy("CSUC.sol", abi.encodeCall(CSUC.initialize, ()));
        csuc = CSUC(proxy);

        csuc.updateConfig(_csucConfig);

        // Aggregator: Main contract, handler, and verifiers
        CurvyAggregator_Types.ConfigurationUpdate memory _aggregatorConfig = CurvyAggregator_Types.ConfigurationUpdate({
            ...
            withdrawBps: 80, // 0.8% fee
            operator: operator,
            csuc: address(csuc)
        });

        proxy = Upgrades.deployUUPSProxy(
            "CurvyAggregator_NoAssetTransfer.sol", abi.encodeCall(CurvyAggregator_NoAssetTransfer.initialize, ())
        );
        aggregator = CurvyAggregator_NoAssetTransfer(proxy);
        aggregator.updateConfig(_aggregatorConfig);

        aggregatorActionHandler = new CurvyAggregator_CSUC_NoAssetTransfer_ActionHandler();

        // Linking the Aggregator handler to the CSUC contract
        ...
        _actionUpdates[0] = CSUC_Types.ActionHandlingInfoUpdate({
            ...
            info: CSUC_Types.ActionHandlingInfo({
                mandatoryFeePoints: uint16(1301), // 0.0%
                handler: address(aggregatorActionHandler)
            })
        });
    ...
    CSUC csuc;
    CurvyAggregator_NoAssetTransfer aggregator;
    CurvyAggregator_CSUC_NoAssetTransfer_ActionHandler aggregatorActionHandler;
    MockERC20[] m20;
}
