...
contract CSUC_operatorExecute_Sequential_Transfer_UnitTest is CSUC_BoilerPlate {
    function setUp() public {
        CSUC_BoilerPlate.multipleSetUp();
    }

    function test_All_Sequential_MultipleTransfer() public {
        ...
        for (uint256 _expectedNonce = 1; _expectedNonce < N_ACTIONS; ++_expectedNonce) {
            for (uint256 i = 0; i < _nTransfers; ++i) {
                ...
                uint256 _transferAmount = _getCSUCBalance(_csa, _token(i)) / 3;
                uint256 _totalFee = csuc.getMandatoryFee(CSUC_Constants.TRANSFER_ACTION_ID, _transferAmount);

                CSUC_Types.Action memory _action =
                    _prepareTransferAction(_csa, _csaPk, _to, _token(i), _transferAmount, _totalFee);
                _actions[i] = _action;
